...
try:
    ...
    def reserve():
        ...
        elif str(train_name) == "Ato1i" and str(train_no) == "12346":
            booking_func("Ato1ii", 12346)
        elif str(train_name) == "Ato1i" and str(train_no) == "12347":
    ...
    def cancel():
        ...
        print()
        mycursor.execute("SELECT ticket_no FROM reservedseats")
        ticket_nos=mycursor.fetchall()
        for row in ticket_nos:
            if int(ticket_no) not in row:
                print("This is not a booked ticket number")
                break
            else:
    ...
    def query():
        ...
        ticket_nos = mycursor.fetchall()
        for row in ticket_nos:
            if int(ticket_no) in row:
                mycursor.execute("SELECT * FROM reservedseats WHERE ticket_no=%s", (ticket_no,))
                status = mycursor.fetchall()
                print("Your ticket information is:",status)
                break
            else:
                print("This is not a booked ticket number")
                break
    ...
    def reservationMenu():
        while 1:
            ...
            print("3.CHECK STATUS OF A TICKET")
    ...
    print("Cancellation is not possible right after Reservation --- Start again from the beginning")
except TypeError:
    print("This is not the right character. Please enter value as asked!")
finally:
